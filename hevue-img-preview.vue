      ...
      <div class="he-bottom-wrap">
        ...
        <div class="he-imglist-wrap" v-if="thumbnail && multiple">
          <div class="he-imglist" :style="{ left: getThumbLeft() + 'px' }">
            <div
              class="he-img-item"
              v-for="(item, key) in thumbnailsList"
              :key="key"
      ...
      handler(newV) {
        if (newV) {
          this.$nextTick(() => {
            ...
            // 判断用户是否传入缩略图列表，如果没有传入，则用imgList代替
            if (
              !this.thumbnailsList ||
              (Array.isArray(this.thumbnailsList) &&
                this.thumbnailsList.length === 0)
            ) {
              this.thumbnailsList = this.imgList
            }
